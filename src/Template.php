...
class Template
{
    ...
    public function __construct( ConfigInterface $config )
    {
        $this->config = $config;
        $this->_options = array(
            'template_dir' => $config[ 'web_template_path' ] . $config[ 'app.template.template' ], //设置系统模板文件的存放目录
            'cache_dir' => $config[ 'var_path' ] . $config[ 'app.template.cache_dir' ] . DIRECTORY_SEPARATOR . APP_NAME . DIRECTORY_SEPARATOR, //指定缓存文件存放目录
    ...
    public function cache( $file )
    {
        ...
        $auto_update = $this->_options[ 'auto_update' ] ? 0 : 1;
        $headeradd = "\n{$auto_update}\n";
